...
carry meaning: `Cmd-p` for print, `Cmd-s` for save, `Cmd-c` for copy. But the
modifiers themselves are abstractions. Any given operation might use `Cmd`,
`Cmd-Shift`, `Cmd-Option`, or something more elaborate. Under favorable
...
well-designed ones. A mode organized around a clear task distinction does
...
area for looping) needed to resort to a different prefix. The letter `x` was
chosen, in part, because it carries the connotation of "scratch out." The
looping start and end points need bindings for quick adjustment. Each point
...
capacious, memorable, and ergonomic than anything a paradigm organized around
mouse, menus, and modifiers can deliver. LoopLlama is evidence that the
approach can be taken to its logical conclusion. Vi is evidence that it can
sustain millions of users across decades.
...
critique — was truly innovative. Raskin responded by designing the Canon Cat
and eventually writing *The Humane Interface*. Those efforts had their flaws,
but the field was alive to the problem. Then the Windows and Mac operating
systems won commercially, and the exploration largely ended. The major
platform makers have issued new versions for decades with minimal change to
the input model.

That stagnation is the thing worth pausing on. Computing is the field most
...
transformed since the 1980s. The direct, tactile question of how the user tells
