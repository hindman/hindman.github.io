...
off; however, if the route to expertise goes through a thicket of
modifier-based key bindings, few will make the journey.
...
have high error costs (for example, no undo), forcing users into
hyper-diligence — such traits force users to act consciously rather than
automatically.
...
underpinned expertise. The same automaticity that makes experts efficient
makes them particularly vulnerable to mode errors: if you wrongly think you
are in insert-mode in vi, the more letters you type, the more likely you are
to do some damage.
...
The GUI market surrendered by exiling lower-priority operations to menus,
accessible only by mouse. The Cat avoided that move, but only by restricting
what the device could do. It was primarily a word-processing appliance, not a
