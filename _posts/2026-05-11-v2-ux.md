...
excerpt: >
  ...
  its inventor understood. The early history of computing produced genuine
  experimentation and a better alternative: modal, multi-key, grammar-based
  keyboard systems. Then the major platforms consolidated, and the exploration
  stopped.
...
same reasoning applies at the next level down. Many applications need to
support multiple kinds of work. Mapping applications have searching and
exploring, but also direct navigation to a destination. Presentation software
is used to create slides and present them. More relevant for our purposes,
many applications — notably, word processors, text editors, and spreadsheets —
need to support at least two types of work: directly typing text versus
issuing commands for editing and navigation. To be effective, the interfaces
for the different tasks must differ. Modes enter the scene again.
...
extent possible. At this point, however, a strong anti-modes stance has lost
its footing: the debate is no longer whether modes are harmful or should
exist; the terrain shifts to sharper questions.
...
Two classic examples illustrate how the criteria play out:
  ...
  - Caps Lock fails on most of them. Its indicator — a small LED on some
    ...
    the key occupies prime real estate, where an errant pinky can easily land.
  ...
  - Vi's insert and normal modes tell the other story. The task distinction is
    ...
    — is arguably too subtle. On balance, these modes earn their place.
...
similar to the conceptual order that applications supply at a higher level.
...
attempted with its printed key labels: contextual, just-in-time, and
